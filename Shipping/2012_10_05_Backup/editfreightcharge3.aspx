...
<script runat="server">
	...
	Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
      ...
      Dim conn As New DBConnect()
  		...
  		Dim intxFreightChargeID as Integer
  		'Dim strProNumber as String
  		'Dim strProNumber2 as String
  		
    If CType(Request.QueryString("fid"), Integer) <> 0 
  		
			strSQL = "SELECT DISTINCT [xFreightChargeID],[FreightType],[FreightTypeName], [ProNumber2], Pronumber, [FreightGL], " & _	
										" OHORNU,ORACLEBranch, convert(datetime, CONVERT(varchar(10), 20100924), 112) AS ShipDate FROM ORDRHDR " & _		
												" LEFT OUTER JOIN [FreightChargesExt] ON OHFCID = [xFreightChargeID] " & _
														" LEFT OUTER JOIN [FreightCharges] ON OHFCID = [FreightChargeID] " & _
															" LEFT OUTER JOIN CUOrderSubmit ON RISOrder = OHORNU " & _
														" LEFT OUTER JOIN CUBranchXrefOracle ON CUBranch = DestinationBranch " & _
													" LEFT OUTER JOIN FreightType ON FreightTypeID =FreightType " & _ 			
            ...
            If CType(Request.QueryString("fid"), Integer) <> 0 
			    If Datetime.Compare(CDate(rdr.Item("ShipDate")),#9/1/2010#)<0 Then

					    Me._LblFreightTypeLabel.Visible = False
					    Me._DdlFreightType.Visible = False
					    Me._LblGLAcctLabel.Visible = False
					    Me._LblGLAccount.Visible = False
    												
			    End if
			End if
			
			Me._HdnOHORNU.Value = rdr.Item("OHORNU").ToString()												
			Me._HdnORACLEBranch.Value = rdr.Item("ORACLEBranch").ToString()
			
			If IsDBNull(rdr.Item("xFreightChargeID")) Then
			    intxFreightChargeID = 0
			Else 
			    intxFreightChargeID = Cint(rdr.Item("xFreightChargeID"))		
			End if 
			
			If Not IsPostBack() Then
			
					If len(rdr.Item("ProNumber2").ToString()) > 0 Then
							Me._TxtProNumber2.Text = rdr.Item("ProNumber2").ToString()

					ElseIf len(rdr.Item("ProNumber2").ToString()) = 0 and len(rdr.Item("ProNumber").ToString())>0
					
							Me._TxtProNumber2.Text = rdr.Item("ProNumber").ToString()					
					End if					
					
					If Not IsDBNull(rdr.Item("xFreightChargeID")) Then
			...
			If Not IsPostBack() and intxFreightChargeID=0 Then
				Dim strSQL2 As String
				strSQL2 = "INSERT FreightChargesExt(xFreightChargeID, ProNumber2, xOHORNU, xOracleBranch) " & _
								" VALUES(@FreightChargeID, @ProNumber2, @xOHORNU, @xORACLEBranch)"
			
			    Dim cmd2 As New SqlCommand(strSQL2, conn.Connection)
			    cmd2.Parameters.AddWithValue("@FreightChargeID",CType(Request.QueryString("fid"), Integer))
				cmd2.Parameters.AddWithValue("@ProNumber2", _TxtProNumber2.Text)			    			
				cmd2.Parameters.AddWithValue("@xOHORNU", Me._HdnOHORNU.Value)
				cmd2.Parameters.AddWithValue("@xORACLEBranch", Me._HdnORACLEBranch.Value)						
...
'    Protected Sub _BtnSave2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles _BtnSave.Click
...
'    Protected Sub _BtnSaveAndExit2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles _BtnSaveAndExit.Click
...
'    End Sub
	...
	Sub _TxtProNumber2_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
		...
		If IsPostBack() Then
				...
				If len(Me._TxtProNumber.Text)= 0 OR Len(Me._TxtProNumber.Text)< len(Me._TxtProNumber2.Text) Then
				
						Me._TxtProNumber.Text = Left(Me._TxtProNumber2.Text,12)	
				
				ElseIf len(Me._TxtProNumber2.Text)= 0 AND Len(Me._TxtProNumber.Text)>0 Then
   ...
   // alert(1)             
    ...
    <asp:GridView runat="server" 
     ...
     <asp:SqlDataSource runat="server" 
        ...
        UpdateCommand="UPDATE FreightChargesExt SET ProNumber2=@ProNumber2, xOHORNU = @xOHORNU, xOracleBranch=@xOracleBranch WHERE xFreightChargeID=@xFreightChargeID"
        ...
        >
         <UpdateParameters>
            ...
            <asp:ControlParameter 
             ...
             <asp:ControlParameter 
                ControlID="_HdnOHORNU" 
                Type="Int32" 
                Name="xOHORNU" 
                PropertyName="Value" 
                />
            <asp:ControlParameter 
                ControlID="_HdnORACLEBranch" 
                Type="String" 
                Name="xORACLEBranch" 
                PropertyName="Value" 
                />                
      ...
      <asp:SqlDataSource runat="server" 
        ...
        UpdateCommand="UPDATE  FreightChargesExt SET FreightType=@FreightType, xOHORNU = @xOHORNU, xOracleBranch=@xOracleBranch WHERE xFreightChargeID=@xFreightChargeID"
        ConnectionString="<%$ ConnectionStrings:RefronConnectionString %>"
        >
        <UpdateParameters>
            <asp:ControlParameter 
    ...
    <asp:HiddenField ID="_HdnVendorCode" runat="server" />                    
		<asp:HiddenField ID="_HdnOHORNU" runat="server" />		                    
		<asp:HiddenField ID="_HdnORACLEBranch" runat="server" />                                       
    ...
    <asp:Table ID="Table1" runat="server">
        ...
        <asp:TableRow>
            <asp:TableCell>
                ...
                <asp:Label runat="server" ID="_LblFreightTypeLabel"  Text="Freight Type : "/> 
                <asp:DropDownList 
                    ID="_DdlFreightType" 
                    runat="server" 
                    ...
                    EnableViewState = "true"                                   
                    /> 
                    <asp:Label runat="server" ID="_LblGLAcctLabel"  Text="GL Account : "/><asp:Label runat="server" ID="_LblGLAccount"  Text="" style="font-weight:bold;"/>              
...
<%-- <atlasToolkit:PopupControlExtender ID="_PceLookupOrders" runat="server">
    ...
    <asp:RequiredFieldValidator runat="server"
        ID="_Rfv_DdlFreightType"
        ControlToValidate="_DdlFreightType"
        ErrorMessage="Freight Type must be selected"  
        InitialValue="0"
        />
</asp:Content>
